...
content = []
list_of_commands = ['RANDG','print', 'math', 'list', 'random_number', 'append_in_list', 'if', 'var', 'funct', 'return']
list_of_randg_commands = ['print', 'math', 'list', 'random_number', 'append_in_list']
math_list = ['+', "-", "*", "/"]
...
def EXEC_BLOCK(commands):
    for cmd in commands:
        if return_value is not None:
            return
        if cmd in list_of_commands or cmd.startswith('print') or cmd.startswith('math') or cmd.startswith('if') or cmd.startswith('var') or cmd.startswith('func') or cmd.startswith('return') or '(' in cmd:
            RANDG_1(cmd)
    ...
    if command.startswith('var '):
        ...
        if len(parts) == 2:
            ...
            if var_value.startswith('"') and var_value.endswith('"'):
                ...
                try:
                    variables[var_name] = EVAL_EXPRESSION(var_value)
                except:
                    variables[var_name] = var_value
            return True
    return False
    
def PROCESS_FUNCTION_DEF(command):
...
def CALL_FUNCTION(command):
    if '(' in command and command.endswith(')'):
        ...
        if funct_name in functions:
            ...
            funct_data = functions[funct_name]
            for i, param in enumerate(funct_data['params']):
                if i < len(args):
                    arg_value = args[i]
                    if arg_value.startswith('"') and arg_value.endswith('"'):
                        variables[param] = arg_value[1:-1]
                    else:
                        try:
                            variables[param] = EVAL_EXPRESSION(arg_value)
                        except:
                            variables[param] = arg_value
            return_value = None
            ...
            for cmd in body_commands:
                if return_value is not None:
                    break
                RANDG_1(cmd)
            stack_data = function_stack.pop()
            variables = stack_data['variables']
            result = return_value
            return_value = stack_data['return_value']
...
def PROCESS_RETURN(command):
    if command.startswith('return'):
        return_expr = command[6:].strip()
        if return_expr:
            try:
                return_value = EVAL_EXPRESSION(return_expr)
            except:
    ...
    if command.startswith('print'):
        ...
        if command.startswith('math (') and command.endswith(')'):
            try:
                ...
                expression = SUBSTITUTE_VARIABLES(expression)
            ...
            operation = random.choice(math_list)
            if operation == '+':
        ...
        PROCESS_FUNCTION_DEF(command)
        
    elif command.startswith('return'):
        PROCESS_RETURN(command)
...
def READ_RANDG(filename): # Reads .randg file
...
def EXEC_RANDG(filename): # Executes .randg file
        ...
        if command.startswith('if'):
            ...
            if first_line.startswith('if (') and ') {' in first_line:
                ...
                if EVAL_CONDITION(condition):
                    EXEC_BLOCK(block_commands)
        elif command.startswith('funct '):
            RANDG_1(command)  # Function creation
        
        elif command in list_of_commands or command.startswith('print') or command.startswith('math') or command.startswith('var') or command.startswith('return') or '(' in command:
            RANDG_1(command)
...
def RANDG(code_lines):
    try:
        for i in range(code_lines):
            current_command = random.choice(list_of_randg_commands)
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        sys.exit(1)
    filename = sys.argv[1]
    EXEC_RANDG(filename) # Or replace filename with you file path
